...
export function useVideoLoader() {
    ...
    const videoItems = ref<{ id: string; videoUrl: string | null | false }[]>([])
    ...
    async function loadBatch(
        ...
        const results = await Promise.all(
            ...
            try {
                ...
                loadedVideos.value.add(key)
                return { id, videoUrl: false as const }
            }
            })
        )
    ...
    async function loadSequentially(playerSlug: string, period: string | null) {
        ...
        if (!loadedVideos.value.has(key)) {
            try {
                ...
                videoItems.value[currentLoadingIndex.value] = { id, videoUrl: false }
            } finally {
                loadedVideos.value.add(key)
            }
        }
