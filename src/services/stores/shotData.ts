  ...
  getters: {
    ...
    getActiveIds(state): number[] {
      return state.entries
        .filter(this._activeEntriesFilter)
        .map((entry) => entry.id);
    },
  },
  actions: {
    getActiveFrequency() {
      return (
        (this.getActiveEntries.length / this.entries.length) *
        100
      ).toFixed(0);
    },
    getById(id: number) {
    ...
    getInefficiencyByColumn<T extends keyof IShotData>(
      ...
      return this._calculateEfficiency(
        ...
        dataset ?? this.entries,
        (makes, total) =>
          total > 0 ? Math.round(((total - makes) / total) * 100) : 0
      );
    },
    ...
    getLeastEffectiveColumnValue<T extends keyof IShotData>(
      ...
      const sample = data.find(
        (entry) => String(entry[col]) === leastEffective.name
      );
    ...
    getStackedPPPAndFrequencyByActionArea(dataset?: IShotData[]) {
      ...
      const lineSeries = {
        ...
        data: actions.map((action) => {
          ...
          return totalCount > 0
            ? Number((totalPTS / totalCount).toFixed(2))
            : 0;
        }),
    ...
    getMostCommonCombination(): Record<string, string> {
      ...
      const combinationCounts = new Map<
        string,
        { count: number; values: Record<string, string> }
      >();
    ...
    getLeastEfficientByAction(actionLimit: number = 3, areaLimit: number = 2) {
      ...
      return actionFG.map((action) => ({
        action: action.name,
        areas: this.getFGByColumn(
          "Area",
          this.entries.filter(
            (entry) => entry["Offensive Action"] === action.name
          )
        )
          .sort((a, b) => a.value - b.value)
          .slice(0, areaLimit),
    ...
    getAllEntriesExceptTop(limit: number = 3): Record<string, string[]> {
      ...
      for (const param of keyParams) {
        ...
        const allEntries = this.getGroupedData(param, activeEntries).sort(
          (a, b) => b.value - a.value
        );
